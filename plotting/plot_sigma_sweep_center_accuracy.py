...
def _plot_shot_panels(hits_stack, cache_indices, sig, x_est, y_est,
    ...
    for row, (shot_idx, slot) in enumerate(picks):
        ...
        # true/estimated *centre coordinates* is direct. `energy` is per-shot,
        # so index by the original shot number.
        e_shot = float(energy[shot_idx]) if hasattr(energy, '__len__') else float(energy)
        x_centers, y_centers, m5, p5, combined = _compute_score_maps(
            img=img, energy=e_shot, cx=cx_ref, cy=cy_ref,
        )
...
def process_sweep_point(sweep_dir, slug, truth_dir, out_dir):
    ...
    # Raw-channel energy per shot: newer bootstraps draw peak_bin per shot and
    # populate `mean_energy_per_shot`. Older ones stored a single scalar
    # `mock_energy_value`. Prefer the per-shot array; broadcast the scalar
    # over all shots if it's the only thing available.
    if 'mean_energy_per_shot' in truth:
        energy = np.asarray(truth['mean_energy_per_shot'], dtype=float)
    else:
        e_scalar = float(truth.get('mock_energy_value', (25 + 13) * 32))
        energy = np.full(len(x_est), e_scalar, dtype=float)
